...
class Permission
{
    ...
    public function create(array $data): string | false
    {
        ...
        $stmt->bindParam(':deduiresurconges', $this->deduiresurconges, PDO::PARAM_INT);
        $stmt->bindParam(':anneecomptable', $this->anneecomptable, PDO::PARAM_INT);
        $stmt->bindParam(':creepar', $login, PDO::PARAM_INT);
        $stmt->bindParam(':accordeepar', $login, PDO::PARAM_INT);
        $stmt->bindParam(':demande', $this->demande, PDO::PARAM_INT);
        $stmt->bindParam(':indexgroupement', $uniqueId, PDO::PARAM_INT);
        $stmt->bindParam(':indexevenement', $uniqueId, PDO::PARAM_INT);
        $stmt->bindParam(':departement', $this->departement, PDO::PARAM_INT);
    ...
    public function update(array $current, array $new_data): int | bool
    {
        ...
        $this->code = htmlspecialchars(strip_tags($new_data['code'] ?? $current['code']));
        $this->type = htmlspecialchars(strip_tags($new_data['type'] ?? $current['type ']));
        ...
        $this->debut = htmlspecialchars(strip_tags($new_data['debut'] ?? $current['debut']));
        $this->fin = htmlspecialchars(strip_tags($new_data['fin'] ?? $current['fin']));
        $this->note = htmlspecialchars(strip_tags($new_data['Note'] ?? $current['Note']));
        $this->block_pointage = htmlspecialchars(strip_tags($new_data['block_pointage'] ?? $current['block_pointage']));
        $this->ibblockpointage = htmlspecialchars(strip_tags($new_data['IDBlockPointage'] ?? $current['IDBlockPointage']));
