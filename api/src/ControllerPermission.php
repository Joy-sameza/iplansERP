...
class controllerPermission
{
     ...
     *
     * @param Permission $cour The Courrier object.
     */
    public function __construct(private Permission $cour)
    {
    }
    /**
     * Process a request.
    ...
    private function processResourceRequest(string $method, string $id): void
    {
        ...
        switch ($method) {
            ...
            case "PATCH":
                ...
                if ($rows === false) {
                    echo json_encode(["message" => "La permission ne peut pas être modifier"]);
                    break;
                } else {

                    echo json_encode([
                        "message" => "permission $id à été mis à jour",
                        "rows" => $rows
                    ]);
                }
                break;
            ...
            case "DELETE":
                ...
                if ($rows === false) {
                    echo json_encode([
                        "message" => "Le permission n'as pas été supprimer"
                    ]);
                    break;
                }
                echo json_encode([
                    "message" => "permission $id à été supprimer",
                    "rows" => $rows
                ]);
                break;
        ...
        switch ($method) {
            ...
            case "POST":
                ...
                if (!$id) {
                    ...
                    echo json_encode(["errors" => "A permission already exists with that name"]);
                    break;
                }
